...
- **anthropic 0.34.2 to 1.7.0, and httpx 0.27.2 to 0.28.1.** The SDK's 1.x line moved its HTTP layer from `httpx` to `httpx2`, the maintained fork, and raised its Python floor to 3.10 (the project is already pinned to 3.11.9). No application code changed: `services/ai_engine.py` is the only consumer, and none of the removed surface (Text Completions, `temperature` / `top_p` / `top_k`, raw `output_format` dicts, `.with_raw_response`, the renamed exports) is anything it used. `httpx` stays pinned because `services/enrichment.py` uses it directly for the ThreatScan calls and starlette's TestClient needs it; it is no longer shared with the SDK. The two bumps are related only in that anthropic 0.34.2 blocked httpx 0.28: it passed the `proxies` argument into its `httpx.Client` subclass unconditionally, and httpx 0.28 removed that argument, so the pair raised `TypeError` at client construction, which in this codebase is import time. Moving the SDK to 1.x removes the constraint rather than satisfying it.
...
- **A long incident report was cut off mid-JSON and failed with an unreadable error.** `services/ai_engine.py` capped output at 2,000 tokens, and a live call on 2026-09-19 produced a 1,500-token report for a single malicious IP, leaving 500 tokens of headroom. A slightly richer incident would have hit the cap, and the only symptom was a `JSONDecodeError` from the parse, which names neither the cap nor the truncation and reached the caller as a bare 500. Two changes: the cap is now 8,000 tokens, over five times the measured size of a richly populated report and roughly three times a pathological one (raising it is free, since billing is for tokens actually generated, and it is not raised further because the request is not streamed and the ceiling also bounds how long an analyst waits); and `message.stop_reason` is now checked before the content is touched, so a `max_tokens` stop raises an error naming the cap, the truncation and the constant to change. Covered by six tests built on real `anthropic.types.Message` and `TextBlock` objects rather than duck-typed stand-ins, so a future SDK change to the response shape breaks them instead of passing silently.
